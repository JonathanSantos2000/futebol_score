...
$month = $monthSelect;
$year = $yearSelec;
$matches = conexaoAPI("http://api.football-data.org/v4/competitions/2019/matches?dateFrom={$year}-{$month}-{$day}&dateTo={$year}-{$month}-{$day}", $chave);
$arrayGames = array();
...
$ranks = conexaoAPI("http://api.football-data.org/v4/competitions/SA/standings?season={$year}", $chave);
$arrayRank = array();
...
$topPlayers = conexaoAPI("http://api.football-data.org/v4/competitions/SA/scorers?season={$year}", $chave);
...
<main>
    <div class="col-left">
        ...
        <div class="rank">
            <h1 class="title-rank">Classificação - Serie A</h1>
            <br>
