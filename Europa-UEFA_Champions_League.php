...
$month = $monthSelect;
$year = $yearSelec;
$matches = conexaoAPI("http://api.football-data.org/v4/competitions/2001/matches?dateFrom={$year}-{$month}-{$day}&dateTo={$year}-{$month}-{$day}", $chave);
$arrayGames = array();
...
$ranks = conexaoAPI("http://api.football-data.org/v4/competitions/CL/standings?season={$year}", $chave);
$arrayRank = array();
...
$topPlayers = conexaoAPI("http://api.football-data.org/v4/competitions/CL/scorers?season={$year}", $chave);
